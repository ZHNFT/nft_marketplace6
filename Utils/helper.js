...
const getCollectionsWithTokenSnippets = async ({ chain, size, sort = 'name', categories, filter }) => {
  const res = await fetch(collectionsUrl({ chain, size, filter, sort, categories: categories?.join(',') }));
