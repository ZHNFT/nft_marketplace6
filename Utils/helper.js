...
const getCollectionsWithTokenSnippets = async ({ chain, size, categories, filter }) => {
  const res = await fetch(collectionsUrl({ chain, size, filter, sort: 'name', categories: categories?.join(',') }));
